...
import api.endpoints.UserEndPonitsProperty;
...
public class UserTestsUsingPropertiesFile {
	...
	{    
		...
		Response response = UserEndPonitsProperty.createNewUser(userpayload);
	...
	{	log.info("***********Reading User Info***********");
		Response response = UserEndPonitsProperty.getUser(this.userpayload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(),200);
	...
	{	
		...
		Response response=UserEndPonitsProperty.updateUser(userpayload, this.userpayload.getUsername());
		response.then().log().all();
		...
		Response responseAfterUpdation = UserEndPonitsProperty.getUser(this.userpayload.getUsername());
		response.then().log().all();
	...
	public void testDeleteUser()
	{
		...
		Response response=UserEndPonitsProperty.deleteUser(this.userpayload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(),200);
